...
module.exports = function saveProfile(heroClass, connection, options) {
    if (!classes[heroClass])
        return Bacon.once(new Bacon.Error("Invalid class, please use one of the following: " + _(classes).keys().join(",")));
    ...
    var host = options.host || 'us.battle.net',
        ...
        seasonStr = season ? ('season/' + season) : 'era/1',
        url = 'http://' + host + '/d3/en/rankings/' + seasonStr + '/rift-' + (hardcore ? 'hardcore-' : '') + heroClass;
    ...
    var observableChain = leaderboardData.flatMapWithConcurrencyLimit(concurrency, function (ranking) {
        ...
        if (debug) {
            profile = profile.doAction(function (p) {
                console.log("[profile] %s", p);
            });
        }
        return profile;
    }).flatMapWithConcurrencyLimit(concurrency, function (profile) {
        var heroes = profile.getBestHeroes(clss, hardcore, season);
        if (heroes.length === 0) {
            if (debug) {
    ...
    if (items)
        observableChain = observableChain.flatMapWithConcurrencyLimit(concurrency, function (hero) {
            return Bacon.fromPromise(hero.fetchItems().return(hero));
        });
    ...
    observableChain = observableChain.mapEnd(function () {
        return Bacon.fromPromise(connection.saveSkills(clss).then(function () {
            if (debug)
