...
module.exports = function (connectionString) {
    ...
    return function (hero) {
        var itemsToInsert = hero.items.map(function (item, slot) {
            return {
                item_id: item.id,
                name: item.name,
                icon: item.icon,
        ...
        return pg('hero').insert({
            ...
            var insertStats = pg('hero_stats')
                .insert(hero.stats.map(function (value, key) {
                    return {
                        hero_id: heroId,
                        name: key,
                        value: value
            ...
            var insertActiveSkills = pg('hero_skills')
                .insert(hero.skills.active.map(function (skill) {
                    return {
                        hero_id: heroId,
            ...
            var insertPassiveSkills = pg('hero_skills')
                .insert(hero.skills.passive.map(function (skill) {
                    return {
                        hero_id: heroId,
                        skill_id: skill.skill.skillCalcId
                    };
