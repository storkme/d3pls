...
module.exports = function (connectionString) {
    ...
    function saveSkill(skill) {
        ...
        if (!isRune) {
            obj[skill.skillCalcId].is_passive = !!(skill.flavor);
        }
        if (skill.type)
            obj[skill.skillCalcId].type = skill.type;
        if (skill.icon)
            obj[skill.skillCalcId].icon = skill.icon;
        if (skill.order)
            obj[skill.skillCalcId].order = skill.order;
        if (skill.categorySlug)
            obj[skill.skillCalcId].category_slug = skill.categorySlug;
        if (skill.flavor)
            obj[skill.skillCalcId].flavor = skill.flavor;
    }
