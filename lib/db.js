        ...
        saveHero: function (hero) {
            ...
            return pg('d3i.hero').select('*').where('hero_id', hero.id)
                .then(function (rows) {
                    if (rows.length > 0) {
                        ...
                        return pg('d3i.hero').where('hero_id', hero.id)
                            ...
                            .then(function () {
                                ...
                                return Promise.join(
                                    pg('d3i.hero_item').where('hero_id', id).del(),
                                    pg('d3i.hero_stats').where('hero_id', id).del(),
                                    pg('d3i.hero_skills').where('hero_id', id).del()
                                );
                            }).return(rows[0].id);
                    } else {
                    ...
                    return Promise.all(insertItems, insertStats, insertActiveSkills, insertPassiveSkills);
                });
        },
        saveSkills: function (clss) {
            var skills = _(allSkills).chain().values().map(_.values).flatten().value();
            return pg('d3i.skills').where('class', clss).del().then(function () {
