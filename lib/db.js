...
module.exports = function (connectionString) {
    ...
    return function (hero) {
        var itemsToInsert = Array.prototype.map(hero.items, function (item, slot) {
            return {
                item_id: item.id,
                name: item.name,
                icon: item.icon,
        ...
        return pg('hero').insert({
            ...
            var insertStats = pg('hero_stats')
                .insert(Array.prototype.map(hero.stats, function (value, key) {
                    return {
                        hero_id: heroId,
                        name: key,
                        value: value
            ...
            var insertActiveSkills = pg('hero_skills')
                .insert(Array.prototype.map(hero.skills.active, function (skill) {
                    return {
                        hero_id: heroId,
            ...
            var insertPassiveSkills = pg('hero_skills')
                .insert(Array.prototype.map(hero.skills.passive, function (skill) {
                    return {
                        hero_id: heroId,
                        skill_id: skill.skill.skillCalcId
                    };
