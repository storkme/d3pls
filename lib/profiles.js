...
var classMap = {
    dh: 'demon-hunter',
    wd: 'witch-doctor'
}, items = ['head', 'torso', 'feet', 'hands', 'shoulders', 'legs', 'bracers',
    'mainHand', 'waist', 'rightFinger', 'leftFinger', 'neck'];

function hasAllSkills(hero) {
    return hero.skills.active.length === 4
        && _(hero.skills.active).every(function (skill) {
            return skill.skill && skill.rune
        })
        && hero.skills.passive.length === 4;
}

function hasItems(hero) {
    return _(items).every(function(slot) {
        return !!hero.skills[slot];
    });
}
...
Profile.prototype.getHero = function (id) {
    ...
    return request.getAsync(this.baseUrl + '/profile/' + this.name + '-' + this.code + '/hero/' + id)
        .spread(function (response, body) {
            return new Hero(profile, JSON.parse(body));
        });
};
